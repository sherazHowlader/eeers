<?php 
    class damageModel extends MainModel{
        ...
        public function readValue($table, $company_name){
            $sql = "SELECT *,sum(dm_quantity) as sumStore,sum(dm_value) as sumValue  
                    FROM $table
                    WHERE company_name='$company_name' AND dm_value <> 0
                    GROUP by pr_name order by pr_name";           
        ...
        public function readAll($table, $company_name){
            $sql = "SELECT SUM(dm_value) as totalPrice
                    FROM $table                    
                    WHERE company_name='$company_name'";
